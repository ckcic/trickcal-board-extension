...
- `npm run build`는 TypeScript 타입 검사 후 번들을 생성합니다. esbuild 단독 성공을 타입 검사 성공으로 간주하지 않습니다.
- 통신, 필터, DOM 캐시를 변경한 경우에도 `npm test`를 실행합니다. 초기 수신 전후 순서, XHR 재사용, 동일 필터의 새 데이터 갱신, 원본 카드 내부 교체를 회귀 테스트에 포함합니다.
- 외부 API의 중첩 구조는 런타임에 검증하며, 보드 차수의 객체·배열 형식을 모두 지원합니다. 개인 데이터 대신 가명 테스트 데이터를 사용합니다.
- 원본 카드에 확장 클래스나 속성이 붙었다는 이유로 원본 DOM 변경까지 감시 대상에서 제외하지 않습니다.
- 배지 캐시는 필터뿐 아니라 진행도 데이터 변경도 반영해야 합니다.
